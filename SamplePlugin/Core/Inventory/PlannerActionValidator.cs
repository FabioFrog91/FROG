...
public sealed class PlannerActionValidator
{
        ...
        out string reason)
    {
        ...
        if (action.Source == action.Destination)
        {
            return Fail(
                out reason,
                "Source and destination cannot be the same storage.");
        }

        if (!IsCharacterAccessible(
                state.CurrentCharacterId,
                action.Source))
        {
            return Fail(
                out reason,
                "The source is not accessible from the current character.");
        }
                ...
                action.Destination))
        {
            return Fail(
                out reason,
                "The destination is not accessible from the current character.");
        }
                ...
                action.Destination))
        {
            return Fail(
                out reason,
                "The requested storage-to-storage route is not physically legal.");
        }
        ...
        out string reason)
    {
        ...
        if (action.FromCharacterId == 0 ||
            action.ToCharacterId == 0)
        {
            return Fail(
                out reason,
                "Character switch requires valid character IDs.");
        }
        ...
        if (action.FromCharacterId != state.CurrentCharacterId)
        {
            return Fail(
                out reason,
                "The switch source does not match the current character.");
        }
        ...
        if (action.FromCharacterId == action.ToCharacterId)
        {
            return Fail(
                out reason,
                "Cannot switch to the same character.");
        }
